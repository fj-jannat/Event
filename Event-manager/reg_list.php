<?php
	...
	$sql = "SELECT * from event_request NATURAL JOIN event JOIN members where Cust_id=memberId order by Req_date desc";
	$res = $conn->query($sql);
				  ...
				  <table id = "example" class="row-border display compact hover" cellspacing="0">
					<thead>
					  <tr id = "firstrow">
						...
						<th><center>Req Status</center></th>
					...
					<tbody>
					   <?php
						...
						?>
						  <td>
							<a href="event_reg_accept.php?accept_id=<?php echo $row['Serial']; ?>" onclick = "return confirm('Sure to Accept?'); " data-toggle="tooltip" title="Accept Request"><i class = "fa fa-check"></i></a>
							<a href="event_reg_reject.php?reject_id=<?php echo $row['Serial']; ?>" onclick = "return confirm('Sure to Reject?'); " data-toggle="tooltip" title="Reject Request"><i class = "fa fa-ban"></i></a>
							<a href="event_reg_delete.php?delete_id=<?php echo $row['Serial']; ?>" onclick = "return confirm('Sure to Delete from Database?'); " data-toggle="tooltip" title="Delete From Database"><i class = "fa fa-trash-o"></i></a>
						  </td>
						</tr>
						<?php
						  }
